...
def get_features_responses_correlation(dataset: pd.DataFrame,
                                       features: list[str],
                                       responses: list[str],
                                       method: str = 'spearman') -> pd.DataFrame:
    """" Computes the correlation between a list of features and a list of responses """
    ...
    for target in responses:
        correlation_list = []
        for xc in features:
            db = pd.DataFrame.from_dict({'x' : dataset[xc],
                                         'y' : dataset[target] })
            db.dropna(inplace=True)
            correlation_list.append(db.corr(method=method).iloc[0,1])
        correlation_dict[target] = correlation_list
    correlation_df = pd.DataFrame(correlation_dict)
    correlation_df['Feature'] = features
    correlation_df = correlation_df[['Feature'] + responses]
...
def variation_of_information(X: np.ndarray,
    ...
    if number_of_bins is None:
        ...
        number_of_bins = discretization_optimal_number_of_bins(X.shape[0], correlation=correlation_xy)
